...
export function CartModal({ locale }: { locale: ContentLocale }) {
  ...
  return (
      ...
      <SheetTrigger asChild>
        <OpenCartButton locale={locale} quantity={totalQuantity} />
      </SheetTrigger>
      ...
      <SheetContent className="flex flex-col">
        <SheetHeader>
          ...
          <SheetDescription>{copy.cartDescription}</SheetDescription>
        </SheetHeader>

        {!cart || cart?.items?.length === 0 ? (
          <div className="text-center flex flex-col items-center gap-2">
            <ShoppingCart className="h-16" />
            <p className="text-center text-2xl font-bold">{copy.cartEmpty}</p>
          </div>
        ) : (
            ...
            <div className="flex flex-col justify-between w-full">
                ...
                {cart?.items?.map((item, i) => {
                  ...
                  if (isVariant) {
                    ...
                    const imageVariant = product.gallery?.find((galleryItem: GalleryItem) => {
                      ...
                      const hasMatch = variant?.options?.some((option: VariantSelection) => {
                    ...
                    <li className="flex w-full flex-col" key={i}>
                      <div className="relative flex w-full flex-row justify-between px-1 py-4">
                        ...
                        <Link
                          ...
                          href={getLocalizedHref(locale, `/products/${(item.product as Product)?.slug}`)}
                        >
              ...
              <div className="px-4">
                <div className="py-4 text-sm text-neutral-500">
                  ...
                  <Button asChild>
                    <Link className="w-full" href={getLocalizedHref(locale, '/checkout')}>
