...
import { getLocalizedHref, resolveLocale } from '@/i18n/routing'
    ...
    <React.Fragment>
      ...
      <div className="container pt-8 pb-8">
        <Button asChild variant="ghost" className="mb-4">
          <Link href={getLocalizedHref(locale, '/shop')}>
            <ChevronLeftIcon />
            All products
          </Link>
        </Button>
        <div className="flex flex-col gap-12 rounded-lg border p-8 md:py-12 lg:flex-row lg:gap-8 bg-primary-foreground">
          <div className="h-full w-full basis-full lg:basis-1/2">
            ...
            >
              {Boolean(fallbackGallery?.length) && <Gallery gallery={fallbackGallery} />}
            </Suspense>
          </div>
      ...
      {product.layout?.length ? <RenderBlocks blocks={product.layout} locale={locale} /> : <></>}
...
function RelatedProducts({ locale, products }: { locale: ContentLocale; products: Product[] }) {
        ...
        {products.map((product) => (
          ...
          >
            <Link
              ...
              href={getLocalizedHref(locale, `/products/${product.slug}`)}
            >
                <GridTileImage
                  label={{
